...
public class BullyBot extends ChickenBot {
  ...
  public int [] fireCannon() {
    ...
    int newEnergyLevel = energyLevel - 2;
    if( count > newEnergyLevel) {
      count = newEnergyLevel;
    }
    ...
    if( count > 0 ) {
      ...
      for( int i = District.NORTH; i < botsSensed.length; i++ ) {
        boolean districtIndex = i != District.CURRENT;
        boolean botsNearBy = botsSensed[i];
        if( districtIndex && botsNearBy && ( j < count ) ) {
          fire[j] = i;
          j++;
          energyLevel--;
        }
